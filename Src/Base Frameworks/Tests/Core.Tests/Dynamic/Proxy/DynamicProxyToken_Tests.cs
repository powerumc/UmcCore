...
namespace Umc.Core.Dynamic.Proxy
{
	...
	public class DynamicProxyToken_Tests
	{
		...
		{
			DynamicProxyToken token1 = new DynamicProxyToken();
			DynamicProxyToken token2 = new DynamicProxyToken();
		...
		{
			DynamicProxyToken token1 = new DynamicProxyToken();
			DynamicProxyToken token2 = this.AssignToken(token1);
		...
		{
			DynamicProxyToken token1 = new DynamicProxyToken();
			DynamicProxyToken token2 = this.RefAssignToken(ref token1);
